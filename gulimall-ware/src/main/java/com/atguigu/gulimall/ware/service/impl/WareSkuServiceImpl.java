...
public class WareSkuServiceImpl extends ServiceImpl<WareSkuDao, WareSkuEntity> implements WareSkuService {
    ...
    public List<SkuHasStockVo> getSkusHasStock(List<Long> skuIds) {
        System.out.println("2.skuIds:" + skuIds);
        List<SkuHasStockVo> collect = skuIds.stream().map(skuId -> {
            SkuHasStockVo vo = new SkuHasStockVo();
            Long count = baseMapper.getStuStock(skuId);
            vo.setSkuId(skuId);
            vo.setHasStock(count == null ? false : count > 0);
            System.out.println("3.vo.skuId:" + skuId + "  vo.HasStock" + vo.getHasStock());

            return vo;
        }).collect(Collectors.toList());
        System.out.println("4.返回collect:" + collect);
        return collect;
    ...
    public Boolean orderLockStock(WareSkuLockVo vo) {
        ...
        for (SkuWareHasStock hasStock : collect) {
            ...
            if (wareIds == null || wareIds.size() == 0) {
                throw new NoStockException(String.valueOf(skuId));
            }
